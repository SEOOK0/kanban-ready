  ...
  const header =
    card.status === "draft"
      ...
      : card.status === "agent_working"
        ? "다음은 칸반 Agent Working 카드다. 에이전트가 spec을 다듬는 중. 본문을 보강해서 Ready로 넘길 준비를 해줘."
        : card.status === "ready"
          ? "다음은 칸반 Ready 카드다. 이 스펙대로 구현해줘. 끝나면 보드에서 Done 컬럼으로 이동시킬 것."
          : "다음은 칸반 Done 카드다. 참고용.";
  const idLine = `--- Card #${card.number} id: ${card.id} (${card.status}) ---`;
  const titleLine = `# #${card.number} ${card.title}`;
  return `${header}\n\n${idLine}\n${titleLine}\n\n${card.body}\n`;
}
